...
public:
    ...
    const Point* getNearestNaive(const Point& point);
    void getKNearestNaive(const Point& point, int k, std::vector<const Point*>& kNearest);
...
private:
    ...
    const Point* getNearestVisitNaive(const Point& point, bool markVisit = false);
